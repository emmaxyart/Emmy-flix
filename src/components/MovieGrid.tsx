...
export default function MovieGrid({ movies, showVideo = true }: MovieGridProps) {
  ...
  const handleStream = async (e: React.MouseEvent, movieId: string) => {
    e.preventDefault();
    if (!session) {
      // Auth disabled - streaming will fail but let the API handle the error
    }
              ...
              {showVideo && (
                <div className="flex gap-2 mt-2">
                  <button
                    ...
                    className="bg-red-600 text-white px-3 py-1 rounded text-sm hover:bg-red-700 transition-colors w-full disabled:opacity-50 disabled:cursor-not-allowed"
                  >
